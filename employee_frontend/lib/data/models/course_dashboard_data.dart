...
class CourseDashboardData {
  ...
  static DashboardCourseStatus statusFor(Course course, {DateTime? now}) {
    ...
    final hasStarted = course.moduleProgress.values.any(
      (progress) => progress.videoWatched || progress.quizPassed,
    );
    final isPassed = modules.isNotEmpty &&
        modules.every(
          (module) {
            final progress = course.moduleProgress['${module.moduleNumber}'];
  ...
  static double progressFor(Course course) {
    ...
    final completed = course.publishedModules.where(
      (module) {
        final progress = course.moduleProgress['${module.moduleNumber}'];
        return progress?.quizPassed == true ||
            (module.quiz.isEmpty && progress?.videoWatched == true);
  ...
  static int watchedModulesFor(Course course) {
    return course.publishedModules
        .where(
          (module) =>
              course.moduleProgress['${module.moduleNumber}']?.videoWatched ==
              true,
        )
        .length;
  }
