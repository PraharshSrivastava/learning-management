...
class FullCourseGenerationNotifier extends StateNotifier<FullCourseGenerationState> {
  FullCourseGenerationNotifier() : super(FullCourseGenerationState(status: FullCourseGenStatus.idle));
  ...
  Future<void> generateFullCourse(String courseId, WidgetRef ref) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final updatedCourse = Course.fromJson(decoded);
        ref.read(selectedCourseProvider.notifier).state = updatedCourse;
        ref.read(currentTabProvider.notifier).state = 2; // Navigate to Courses tab (formerly Training tab)
      } else {
...
final fullCourseGenerationProvider =
    StateNotifierProvider<FullCourseGenerationNotifier, FullCourseGenerationState>((ref) {
