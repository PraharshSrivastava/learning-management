...
class _QuizViewState extends ConsumerState<QuizView> {
                    ...
                    if (context.mounted) {
                      ...
                      if (state.status == QuizGenStatus.success) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content:
                                Text('Quiz generation failed: ${state.error}'),
  ...
  Future<void> _openManualQuizDialog(CourseModule module) async {
    ...
    final success =
        await ref.read(courseUpdateProvider.notifier).saveModuleQuiz(
              widget.course.courseId,
              module.moduleNumber,
              questions,
              ref,
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
            success ? 'Manual quiz saved.' : 'Could not save manual quiz.'),
...
class _ManualQuizDialogState extends State<_ManualQuizDialog> {
  ...
  void initState() {
    super.initState();
    final count =
        widget.module.numQuestions > 0 ? widget.module.numQuestions : 3;
