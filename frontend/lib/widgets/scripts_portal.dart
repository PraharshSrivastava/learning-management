...
class _ScriptsViewState extends ConsumerState<ScriptsView> {
  ...
  Widget build(BuildContext context) {
    if (widget.course.modules.isEmpty) {
      return const Center(child: Text('No modules available. Please blueprint outline first.'));
    }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () {
                ref.read(currentTabProvider.notifier).state = 3; // Navigate to Slides tab
              },
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              onPressed: () async {
                await ref.read(scriptGenerationProvider.notifier).generateScripts(widget.course.id, ref);
              },
        ...
        Expanded(
          child: ListView.separated(
            ...
            itemBuilder: (context, idx) {
              ...
              final isPlayingThis = _playingSlideIndex == idx && _isAudioPlaying;
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      const Icon(Icons.description_outlined, color: AppTheme.primaryBlue, size: 18),
                                  ...
                                  if (hasAudio)
                                    IconButton(
                                      iconSize: 32,
                                      color: isPlayingThis ? AppTheme.accentGreen : AppTheme.primaryBlue,
                                      icon: Icon(
                                        isPlayingThis
                                            ? Icons.pause_circle_filled_rounded
                                            : Icons.play_circle_filled_rounded,
                                      ),
                                      onPressed: () => _togglePlayAudio(idx, audioPath),
                                      tooltip: isPlayingThis ? 'Pause Narration' : 'Listen to Narration',
                                    ),
                                ],
                              ),
  ...
  Widget _buildSlideOutlineDetails(Map<String, dynamic> slide) {
    ...
    if (layoutStr == 'concept' && slide['concept_data'] != null) {
      ...
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Concept layout', style: GoogleFonts.barlow(fontSize: 11, fontWeight: FontWeight.bold, color: AppTheme.gray)),
          const SizedBox(height: 4),
          Text(coreTerm, style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.bold, color: AppTheme.primaryBlue)),
          const SizedBox(height: 4),
          Text(definition, style: GoogleFonts.barlow(fontSize: 13, color: AppTheme.textBlack)),
          if (takeaways.isNotEmpty) ...[
            const SizedBox(height: 8),
            Text('Takeaways:', style: GoogleFonts.barlow(fontSize: 11, fontWeight: FontWeight.bold, color: AppTheme.gray)),
            ...takeaways.map((t) => Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Expanded(child: Text(t.toString(), style: GoogleFonts.barlow(fontSize: 12.5))),
                ],
              ),
            )),
          ],
      ...
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Timeline layout', style: GoogleFonts.barlow(fontSize: 11, fontWeight: FontWeight.bold, color: AppTheme.gray)),
          const SizedBox(height: 6),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(step['title'] ?? '', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.bold)),
                      Text(step['description'] ?? '', style: GoogleFonts.barlow(fontSize: 12, color: AppTheme.gray)),
                    ],
                  ),
                ),
              ],
      ...
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Comparison layout', style: GoogleFonts.barlow(fontSize: 11, fontWeight: FontWeight.bold, color: AppTheme.gray)),
          const SizedBox(height: 6),
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(leftTitle, style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.bold, color: AppTheme.primaryBlue)),
                    const Divider(height: 8),
                    ...leftPoints.map((p) => Text('• ${p.toString()}', style: GoogleFonts.barlow(fontSize: 12))),
                  ],
                ),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(rightTitle, style: GoogleFonts.inter(fontSize: 12.5, fontWeight: FontWeight.bold, color: AppTheme.accentOrange)),
                    const Divider(height: 8),
                    ...rightPoints.map((p) => Text('• ${p.toString()}', style: GoogleFonts.barlow(fontSize: 12))),
                  ],
                ),
              ),
            ],
      ...
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Grid layout', style: GoogleFonts.barlow(fontSize: 11, fontWeight: FontWeight.bold, color: AppTheme.gray)),
          const SizedBox(height: 6),
          Wrap(
            spacing: 6,
            runSpacing: 6,
            children: columns.map((col) => Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(col['header'] ?? '', style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: AppTheme.primaryBlue)),
                  const SizedBox(height: 2),
                  Text(col['content'] ?? '', style: GoogleFonts.barlow(fontSize: 10.5, color: AppTheme.textBlack)),
                ],
              ),
            )).toList(),
          ),
      ...
      final List<dynamic> bullets = slide['bullets_data'] ?? slide['bullets'] ?? [];
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Bullets list layout', style: GoogleFonts.barlow(fontSize: 11, fontWeight: FontWeight.bold, color: AppTheme.gray)),
          const SizedBox(height: 6),
          ...bullets.map((b) {
            ...
            return Padding(
              ...
              child: Row(
                ...
                children: [
                  const Text('• ', style: TextStyle(fontWeight: FontWeight.bold, color: AppTheme.primaryBlue)),
                  Expanded(child: Text(bText.toString(), style: GoogleFonts.barlow(fontSize: 12.5))),
                ],
              ),
            );
          }),
